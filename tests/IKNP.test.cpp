...
//    std::array<bool, 1> COTChoices {false};
    ...
    std::thread EOTSenderThread {[&k0, &k1, &keys, &COTChoices]() {
        auto rd {ATLab::PRNG_Kyber::get_PRNG_Kyber()};
        for (auto& rawBuf : k0) {
        ...
        socket.accept();
        ATLab::EndemicOT::batch_send(socket, k0.data(), k1.data(), k0.size());
        ...
        sleep(1);
        socket.connect();
        ATLab::EndemicOT::batch_receive(socket, k.data(), b.data(), k.size());
    ...
    for (size_t i {0}; i != keys.size(); ++i) {
        const auto& key {ATLab::as_uint128(keys.at(i))};
        const auto& keyBase {ATLab::as_uint128(keyBases.at(i))};
        if (COTChoices.at(i)) {
...
TEST(IKNP, EOT_Factory) {
    std::array<bool, 128> choicesOT {ATLab::random_bool_array<128>()};
    auto choicesCOT {ATLab::random_bool_array<COT_SIZE>()};
    ...
    std::thread EOTSenderThread {
        [&keysCOT, &choicesCOT]() {
            ATLab::Socket socket(ADDRESS, PORT);
            socket.accept();
            auto receiver {emp::IKNP::receiver_by_EOT(socket, ATLab::PRNG_Kyber::get_PRNG_Kyber())};
    ...
    for (size_t i {0}; i != keysCOT.size(); ++i) {
        const auto& key {ATLab::as_uint128(keysCOT.at(i))};
        const auto& keyBase {ATLab::as_uint128(keyBasesCOT.at(i))};
        if (choicesCOT.at(i)) {
            EXPECT_EQ(key, keyBase ^ delta);
        } else {
            EXPECT_EQ(key, keyBase);
